...
# ==========================================
    ...
    with c2:
        ...
        with b3:
            ...
            st.metric("팩 공칭 전압", f"{battery_vol:.1f} V", f"총 {(battery_capacity / 1000) * battery_vol:.0f} Wh", delta_color="off")
...
# ==========================================
    ...
    if not api_key:
        ...
        if st.session_state.llm_result is None:
            with st.spinner("🧠 LLM 엔진 구동 중... 제약 조건을 분석하여 물리적 한계를 연산하고 있습니다..."):
                try:
                    prompt = f"""
        ...
        # [핵심] LLM의 M 추천값을 바탕으로 Ltx, Lrx 결정론적 역산
...
# ==========================================
    ...
    # 파라미터 가져오기 (Auto면 추천값 그대로, Manual이면 조정한 값)
    t_data = st.session_state.tuning_data
    ...
    if "error" in res:
        ...
        e1.metric("예상 전송 효율", f"{res['efficiency']:.1f} %", delta="발열 통제 가능" if res['efficiency'] >= 90 else "효율 저하 경고", delta_color="normal" if res['efficiency'] >= 90 else "inverse")
        e2.metric("송신 코일 전류 (Itx)", f"{res['Itx']:.2f} A", delta="주의: 30A 초과" if res['Itx'] >= 30 else "안정", delta_color="inverse" if res['Itx'] >= 30 else "normal")
        e3.metric("상호 인덕턴스 (M)", f"{res['M']*1e6:.2f} μH")
        
        st.divider()
        st.subheader("⚡ 보상 소자값 및 내압(Voltage Stress) 분석")
        def display_voltage_metric(label, cap, vol): st.metric(label, f"{cap*1e9:.2f} nF", f"Peak: {vol:.0f} V", delta_color="off")
        
        if "LCC-S" in topology:
            c1, c2, c3 = st.columns(3)
            with c1: display_voltage_metric("Tx 병렬 커패시터 (Cp)", res['Cp'], res['V_Cp_peak'])
            with c2: display_voltage_metric("Tx 직렬 커패시터 (Cs)", res['Cs'], res['V_Cs_peak'])
            with c3: display_voltage_metric("Rx 직렬 커패시터 (Crx)", res['Crx'], res['V_Crx_peak'])
        else:
            c1, c2, c3, c4 = st.columns(4)
            with c1: display_voltage_metric("Tx 병렬 커패시터", res['Clcc_tx'], res['V_parallel_tx_peak'])
            with c2: display_voltage_metric("Tx 직렬 커패시터", res['Cp_tx'], res['V_series_tx_peak'])
            with c3: display_voltage_metric("Rx 병렬 커패시터", res['Clcc_rx'], res['V_parallel_rx_peak'])
            with c4: display_voltage_metric("Rx 직렬 커패시터", res['Cp_rx'], res['V_series_rx_peak'])
        ...
        # [CSV Export 로직] Phase 1 데이터 + 계산 결과 병합
        ...
        with c1:
            st.download_button(label="📊 전체 설계 데이터 다운로드 (.csv)", data=csv_data, file_name="wpt_full_report.csv", mime="text/csv", type="primary", use_container_width=True)
        with c2:
            st.button("🔄 처음부터 다시 설계하기", on_click=go_to_step, args=(0,), use_container_width=True)
